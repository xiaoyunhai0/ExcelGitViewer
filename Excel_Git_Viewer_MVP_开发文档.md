...
1. 导入本地 Git 仓库。
2. 显示最近的提交记录。
3. 只显示当前分支可达的普通提交，默认只显示改动过 `.xlsx` 的提交。
...
默认只读取当前 `HEAD` 可达、修改过 `.xlsx` 的普通提交。使用空字符作为字段分隔符，避免提交信息、中文和空格影响解析：

```bash
git -C <repo> log -n 200 --no-merges --date=iso-strict \
...
默认加载当前分支最近 200 条改动过 `.xlsx` 的普通提交，并提供“显示全部普通提交”开关。
...
同一个 Git Blob 内容不会变化，可以按 Blob ID 缓存解析结果：

```text
cache key = git_blob_sha
```
...
MVP 可以先使用进程内 LRU 缓存，不必实现持久化数据库。
...
1. 能导入一个有效的本地 Git 仓库。
2. 能显示当前分支最近 200 条改动过 `.xlsx` 的普通提交记录，可切换显示全部普通提交。

...
1. 导入本地 Git 仓库。
2. 显示最近的提交记录。
3. 有界读取当前分支最近 1000 个普通提交的元数据，显示最近 200 条，不按文件类型筛选提交。
...
一次只读取当前 `HEAD` 可达的最近 1000 个普通提交元数据。使用空字符和固定数量的提交字段解析，避免提交信息、中文和空格影响结果；历史加载不读取任何提交的变更文件路径：
...
默认只读取当前分支最近 1000 条普通提交的元数据，并显示最近 200 条。提交列表不按 Excel 变更筛选；点击提交后再读取其中变化的 `.xlsx` 文件。
...
1. 能导入一个有效的本地 Git 仓库。
2. 能读取当前分支最近 1000 条普通提交的元数据并显示最近 200 条，历史加载不读取变更文件路径。

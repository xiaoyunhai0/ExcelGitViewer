...
├──────────────────┬─────────────────┬───────────────────────────────┤
│ 提交记录（表格） │ Excel 文件      │ 工作表概览                    │
│ 哈希 说明 作者… │ 状态 文件名     ├───────────────────────────────┤
│ a82… 修改… 张三 │ M 活动策划.xlsx │ 差异记录：位置 / 旧值 / 新值 │
│ 91c… 新增… 李四 │ A 新活动.xlsx   ├───────────────┬───────────────┤
│                  │                 │ 修改前上下文  │ 修改后上下文  │
└──────────────────┴─────────────────┴───────────────┴───────────────┘
```
...
提交记录、Excel 文件和差异详情之间可左右拖动；工作表概览、差异记录和上下文之间
可上下拖动；修改前、修改后上下文之间可左右拖动。窗口变化时各区域随窗口扩展，
退出时保存三组分隔比例，下次启动自动恢复。
...
14. 主栏可左右缩放，详情区可上下缩放，新旧上下文可左右缩放，退出后恢复分隔比例。
...
18. 工作区支持横向与纵向拖动缩放，并在重启后恢复用户设置的分隔比例。

...
1. 导入本地 Git 仓库。
2. 显示最近的提交记录。
3. 有界扫描当前分支最近 1000 个普通提交，默认显示其中改动过 `.xlsx` 的提交，最多显示 200 条。
...
│ 提交记录       │ Excel 文件      │ 差异详情                  │
│                │                 │                           │
│ a82fd31        │ M 活动策划.xlsx │ [单元格] [图片]           │
│ 修改活动奖励   │ A 新活动.xlsx   │                           │
│ 张三 10:42     │ D 旧任务.xlsx   │ 工作表：任务配置          │
│                │                 │ B12  100 → 150            │
│ 91cb662        │                 │                           │
│ 新增七日任务   │                 │ 图片1  替换                │
└────────────────┴─────────────────┴───────────────────────────┘
...
一次读取当前 `HEAD` 可达的最近 1000 个普通提交，并同时输出每个提交的变更状态和文件名。使用空字符、固定数量的提交头字段以及状态对应的路径数量解析，避免提交信息、中文、空格和文件名换行影响结果：

```bash
git -C <repo> log -n 1000 --no-merges \
  --pretty=format:%x00%H%x00%P%x00%an%x00%ae%x00%aI%x00%s%x00 \
  --name-status -z
```

模块从这一份有界快照同时生成“全部普通提交”和“Excel 提交”两个视图，各最多 200 条。切换“仅 Excel 提交”只在内存中更换视图，不再执行 Git 命令；首次选择仓库、HEAD 变化或点击“刷新”时才重新扫描。
...
默认扫描当前分支最近 1000 条普通提交，显示其中最近 200 条改动过 `.xlsx` 的提交，并提供“仅 Excel 提交”开关。切换开关必须立即完成，不得重新读取 Git 历史。
...
选中一条单元格变化时，在下方显示该坐标周边少量行列的只读上下文。位于隐藏工作表、隐藏行或隐藏列的变化必须显示标记。字符串变化涉及空格、制表符或换行时，界面提供可见提示。
...
缓存只保存提交 ID、父提交 ID、作者、时间、标题以及由变更路径生成的“全部提交”和
“Excel 提交”视图，不保存 Excel 工作簿内容。以下情况必须重新读取 Git 并覆盖缓存：
...
工作簿解析结果后续可以按 Git Blob ID 增加进程内 LRU 缓存；它与本节的提交历史缓存相互
独立，不是本轮提交历史性能优化的必要条件。
...
7. 超过 2 秒的任务可取消；
8. 快速切换时过期任务不会覆盖当前结果；
9. “仅 Excel 提交”切换只更换内存视图，不执行新的 Git 历史扫描；
10. 启动时自动恢复上次成功打开的仓库；
...
1. 能导入一个有效的本地 Git 仓库。
2. 能在当前分支最近 1000 条普通提交中，分别显示最近 200 条普通提交或最近 200 条 `.xlsx` 提交；切换视图不得重新扫描 Git。

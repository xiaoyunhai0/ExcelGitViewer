...
和 `.xls` 改动。它不只告诉你“二进制文件发生变化”，还会显示工作簿内部具体改了什么。
...
| Excel 文件 | 识别 `.xlsx` 和 `.xls` 文件的新增、修改、删除和重命名。 |
| 工作表 | 报告工作表新增、删除和修改。 |
| 单元格 | 显示值变化、`.xlsx` 公式文本和 `.xls` 保存的公式结果。 |
| 上下文 | 旧值标红、新值标绿，对应行列标题标黄。 |
| 风险提示 | 标记隐藏工作表、隐藏行列和不可见空白。 |

内嵌图片差异属于后续里程碑，目前尚未实现。
对于旧版 `.xls`，解析器无法取得公式原文，因此比较的是文件最后保存的公式计算结果；
`.xlsx` 仍然比较公式文本。
...
| 当前支持 | 暂不支持 |
|---|---|
| 普通提交中的 `.xlsx` 和 `.xls` 文件 | 合并提交和任意两次提交对比 |
| 单元格值和公式文本 | 格式、图表、形状和批注 |
| 根提交与空工作簿比较 | 工作区和未提交改动 |
| Microsoft Excel `.xlsx`/`.xls` 兼容 | 已完成的 WPS 验收 |

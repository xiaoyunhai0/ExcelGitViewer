...
提交/文件导航区和差异详情之间可左右拖动；提交记录和 Excel 文件之间、工作表概览、
差异记录和上下文之间均可上下拖动；修改前、修改后上下文之间可左右拖动。窗口变化时
各区域随窗口扩展，退出时保存四组分隔比例，下次启动自动恢复。同一记录内切换列不应
重复读取文件或解析工作簿。
...
14. 主栏可左右缩放，导航和详情区可上下缩放，新旧上下文可左右缩放，退出后恢复分隔比例。
...
18. 工作区支持横向与纵向拖动缩放，并在重启后恢复用户设置的四组分隔比例。

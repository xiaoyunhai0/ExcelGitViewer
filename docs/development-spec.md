...
5. 列出该提交中新增、修改、删除或重命名的 `.xlsx` 和 `.xls` 文件。
6. 查看工作表变化。
7. 查看单元格值变化；`.xlsx` 比较公式文本，`.xls` 比较最后保存的公式计算结果，并保留空格、制表符和换行。
...
9. 查看 Excel 内嵌图片变化：
   ...
   - 缩放图片。
10. 显示图片修改前和修改后的预览。
11. 支持 Microsoft Excel 生成的 `.xlsx`、`.xls`，以及 WPS 生成的标准 `.xlsx`。
...
模块从这一份有界结果显示最近 200 条普通提交。界面不提供“仅 Excel 提交”筛选，用户选择提交后才读取该提交中的 `.xlsx` 和 `.xls` 文件，避免历史加载和筛选状态影响操作响应。
...
git -C <repo> diff-tree \
  ...
  -- ':(icase,glob)**/*.xlsx' ':(icase,glob)**/*.xls'
```

只保留扩展名为 `.xlsx` 或 `.xls` 的文件，扩展名匹配不区分大小写。
...
`.xls` 根据 OLE Compound File 文件头自动改用 xlrd，并启用 `formatting_info=True` 读取隐藏
工作表、行和列。xlrd 不提供公式原文，因此 `.xls` 只比较文件最后保存的公式计算结果；
不执行公式，也不启动 Excel。单个 `.xls` 输入在解析前限制为 128 MB。
...
默认只读取当前分支最近 200 条普通提交的元数据。提交列表不按 Excel 变更筛选；点击提交后再读取其中变化的 `.xlsx` 和 `.xls` 文件。
...
取消采用协作式语义：界面立即恢复可操作并使当前任务结果失效；解析器在工作表、行和比较边界检查取消令牌。openpyxl 或 xlrd 正在执行的单次工作簿加载无法在线程内安全硬中断，但完成后的结果必须丢弃，不得回写 UI。

---

## 12. 核心模块接口

核心模块应将 Git 命令解析、Excel 读取和标准化细节隐藏在小接口后。UI 和测试使用同一公开接口，不依赖 openpyxl、xlrd 或 Git 输出格式。
...
相同工作簿在相邻提交或重复选择中再次出现时，直接复用快照，不重复执行 openpyxl 或 xlrd
...
4. `.xlsx` 解压前检查 ZIP 条目数量、单条目大小和总解压大小，防止 ZIP Bomb；`.xls` 解析前检查文件总大小。
...
完成标准：能从任意普通提交取到旧、新两个 `.xlsx` 或 `.xls` 字节流。
...
12. 点击提交时只按 `.xlsx` 和 `.xls` 路径读取该提交的文件变化。
...
3. 点击提交后能列出该提交变化的 `.xlsx` 和 `.xls` 文件。
4. 能比较当前普通提交和它的唯一父提交。
5. 能显示单元格新增、删除和修改。
6. `.xlsx` 能显示公式旧值和新值；`.xls` 能显示最后保存的公式计算结果变化。
7. 能显示图片新增和删除。
...
openpyxl 和 xlrd 的工作簿加载是同步调用，无法在进程内安全强制终止。里程碑 1 保证取消后 UI 立即恢复、过期结果绝不回写，并在可检查边界尽快停止。如真实上限样本证明包加载本身造成长时间 CPU 或内存占用，后续版本需将解析移入可终止的独立子进程。

...
- 只读取当前分支最近 200 条普通提交的元数据，不扫描历史文件路径。
- 点击提交后才按 `.xlsx` 路径读取该提交的 Excel 文件列表。
- 使用约 384 MB 上限的进程内 LRU 快照缓存，加速重复和相邻工作簿对比。
- 自动记住并重新打开上次使用的仓库；仓库 HEAD 未变化时，重启后复用本地提交历史缓存。
- 识别 Excel 文件的新增、修改、删除和重命名。
- 显示单元格新增、删除、修改以及新旧值。
- 比较公式文本，不执行公式。
- 标记隐藏工作表、隐藏行列和不可见空白字符。
- 使用紧凑表格显示提交、Excel 文件和差异，支持横向与纵向拖动调整各详情区域。
- 显示差异单元格周边的新旧只读上下文，并以红/绿语义色标出当前改动坐标。
- 自动保存界面分隔比例，下次启动继续使用上次布局。
...
程序退出后会记住最后一次打开的仓库。再次启动时，如果该仓库的 HEAD 没有变化，提交列表
会直接从本地缓存恢复；拉取到新提交后会自动重新读取 Git，点击“刷新”也会强制重新读取。
